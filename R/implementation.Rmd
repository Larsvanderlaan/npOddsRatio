...
fit$coefs
fit$logOR_at_W_new
fit$pred_function(data[,c("W1", "W2")])
```
...
for(i in 1:200){
  ...
  D <- D +
    ...
    node("Z", distr = "rnorm", mean = W1 + 0.5*A, sd = 2.5) +
    node("Q", distr = "rconst", const = plogis(A + A*(0.5*W1 + W1^2) + sin(5*W1) + cos(4*W2)))+
    node("Q1", distr = "rconst", const = plogis(1 + (0.5*W1 + W1^2) + sin(5*W1) + cos(4*W2)))+
    ...
    node("Delta", distr = "rbinom", size = 1, prob = 1- 0.3*plogis(W1 + Z + W2 + A))
  true <- c(1,1)
  setD <- set.DAG(D )
  data <- sim(setD, n = n)
  ...
  fit <- npORmissing(~ poly(W1,degree=2, raw = T)   , W = data[,c("W1", "W2")], A = data$A, Y = data$Y , Z = data$Z, Delta = data$Delta, num_knots_Y = c(20,1), max_degree_Y = 2, sl3_learner_Delta = Lrnr_gam$new() , sl3_learner_A = Lrnr_gam$new()  )
  ci <- fit$coefs
